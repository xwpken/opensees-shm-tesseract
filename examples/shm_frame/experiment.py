"""Damage parameters, loading, observations, and analysis for the static frame."""
...
from examples.shm_frame.model import MEMBERS, NODE_COORDINATES, member_nodes
...
SENSOR_NODES = tuple(dict.fromkeys(
    ...
    for node in member_nodes(member)[1:-1]
))
SENSOR_DOFS = (1, 2, 3)
RAW_RESPONSES = tuple(
    (node, dof)
...
def make_opensees_program(nominal_area: float, nominal_inertia: float) -> str:
    """Serialize the eight static tests as one OpenSees-DDM program."""
    ...
    for member_index, (node_i, node_j) in enumerate(MEMBERS):
        ...
        nodes = member_nodes(member_index)
