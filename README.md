...
Neither library forms part of the native `JAX` computation graph. The geometry and section finite-element calculations in `sectionproperties` execute outside the `JAX` tracing model, while `OpenSees` is a stateful external solver with its own sensitivity machinery. A differentiable inference workflow must therefore combine derivatives generated by different numerical mechanisms across both boundaries:
...
Let the section-analysis `Tesseract` be $\mathcal S$, the `OpenSees` `Tesseract` be $\mathcal O$, and the `JAX` observation operator be $\mathcal H$. The complete forward model is

```math
\boldsymbol p = \mathcal S(\boldsymbol\theta),
\qquad
...
└──────────────────────────────────────────────┘
          │  section and element properties p
          ▼
┌──────────────────────────────────────────────┐
│ Tesseract B — structural analysis            │
│ OpenSees: model + analysis + responses       │
└──────────────────────────────────────────────┘
          │  response history r
          ▼
┌──────────────────────────────────────────────┐
...
The `section-properties` Tesseract differentiates the corrosion-to-area map, the `opensees-ddm` Tesseract uses `OpenSees` DDM for the area-to-displacement map, and `JAX` differentiates the hysteretic-work calculation. The reference gradient perturbs the original corrosion parameters and repeats the entire section and structural analyses, so the comparison tests the complete composition rather than either component in isolation.
